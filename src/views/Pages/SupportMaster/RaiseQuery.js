...
const RaiseQuery = (props) => {
    ...
    useEffect(() => {
        cabretApi.get('/api/feedback/feedbackType/Query').then(res => {
            console.log(res)
    ...
    useEffect(() => {
        setLoading(true)
        cabretApi.get('/api/feedback/feedback-number/Query').then(res => {
            console.log(res)
            setLoading(false)
    ...
    const HandleChange = (value, id) => {
        ...
        const newData = queryData.map((el) => el._id === id ? { ...el, status: value } : el)
    ...
    const dataInput = queryData && queryData.length>0 ? queryData.map((item, index) => {
        return {
            sno: index+1,
            ...
            // query: item.feedback ? item.feedback : "NA",
            status: item.status ? item.status : 'Select Status',
        }
    }) : [];
    const dataCallback = [
        { sno: 1, data: '24-07-2020 10:30 A.M.', name: 'Partner Name', email: 'partneremialid@gmail.com', contact: '1234567890', query: 'Content of the Query', status: 'reopen' },
        { sno: 2, data: '24-07-2020 10:30 A.M.', name: 'Partner Name', email: 'partneremialid@gmail.com', contact: '1234567890', query: 'Content of the Query', status: 'reopen' },
    ];
